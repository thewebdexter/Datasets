...
DOMAIN_MAP_FILE = OUTPUT_DIR / "domain_map.json"   # compact file for the extension
...
def feed_urlhaus() -> list[dict]:
    """URLhaus (abuse.ch) – malware distribution URLs (plain CSV, not gzipped)."""
    log.info("URLhaus …")
    try:
        # abuse.ch serves plain CSV with ## comment lines — NOT gzipped
        ...
        for row in reader:
            ...
            tag = (row.get("tags") or "malware").strip() or "malware"
            if url:
...
def feed_spamhaus_dbl() -> list[dict]:
    """Hagezi spam + fraud domains list (mirrors Spamhaus DBL coverage)."""
    log.info("Spamhaus DBL / Hagezi spam …")
    url = (
        "https://raw.githubusercontent.com/hagezi/dns-blocklists/main"
        "/domains/spam-mails.txt"
    )
    try:
        r = fetch(url)
        results = []
...
def build():
    ...
    for entry in all_entries:
        ...
        # Threat categories win over known_good if the same domain appears in both
        if domain and (domain not in seen_domains or entry["category"] != "known_good"):
            seen_domains[domain] = entry["category"]

    log.info("Unique entries after dedup: %d  |  unique domains: %d", len(clean), len(seen_domains))

    now = datetime.now(timezone.utc).isoformat()

    # ── Write compact domain map (what the extension loads) ────────────────
    # Just {domain: category} — no full URL list, stays well under GitHub's 100 MB limit
    DOMAIN_MAP_FILE.write_text(
        json.dumps(seen_domains, separators=(",", ":")), encoding="utf-8"
    )
    log.info("Wrote %s (%.1f MB)", DOMAIN_MAP_FILE, DOMAIN_MAP_FILE.stat().st_size / 1e6)

    # master_feed.json is intentionally NOT written here — at 1M+ entries it
    # exceeds GitHub's 100 MB hard limit and would break the push every run.
